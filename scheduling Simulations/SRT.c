    ...
    for (int o = 1; o <= 25; o++){
        ...
        while (!current || t < k || !pheap_is_empty(burst_times)) {
            while (t == get_min(arrival_times)) {     //new process arrives
                ...
                printf("t = %d: A new process admitted, bt = %d. Current pheap min time = %d\n", t, p.burst_time, pheap_get_min(burst_times).burst_time);
            }
            ...
            if (current != NULL) {
                if (pheap_get_min(burst_times).burst_time < current->burst_time){   // reinsert current process to heap
                    pheap_min_insert(&burst_times, *current);
                    *current = pheap_min_delete(&burst_times);
                }
