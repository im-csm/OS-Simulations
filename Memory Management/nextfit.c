...
int nextFit(int* blocks, int* block_count, int* mem, int d, double v, int n, int iteratorVal){
    if(head == -1)//no hole is found!
        return 0;
    int size = nrand_gen(d, v, n);  // Generate random size for block
    int iterator = iteratorVal, pred, succ, new_size, block_address;



    // Find the next hole to fit the new block, starting from where the search left off.
    while(size > -mem[iterator]){
        number_of_examined_holes++;
        iterator = mem[iterator + 2];   // iterator = pointer to next hole
        if(head == iterator)
...
void release(int* blocks, int* block_count, int* mem, int arrSize){
    ...
    if (leftLen >= 0 && rightLen >= 0){
        ...
        *pRight = blockSize;                        // Join rightmost hole with new hole
        ...
        int pred = mem[rightLen + 1], succ = mem[rightLen + 2];
        if (rightLen == 0){
            mem[blockAddress] = mem[ (blockAddress + newSize + 1) % arrSize] = -newSize;
            mem[blockAddress + 1] = pred;
            mem[blockAddress + 2] = rightLen;
            mem[pred + 2] = blockAddress;
            mem[rightLen + 1] = blockAddress;
        }
        else {
            newSize = -mem[blockAddress] + 2 + -mem[rightLen];
            mem[blockAddress] = mem[(blockAddress + newSize + 1) % arrSize] = -newSize;
            mem[blockAddress + 1] = pred;
            mem[blockAddress + 2] = succ;
            mem[pred + 2] = blockAddress;
            mem[succ + 1] = blockAddress;
        }
    }
...
int main(int argc, char** argv) {
    ...
    while(x > 0){
        while((iterate = nextFit(blocks, &block_count, mem, d, v, n, iterate)));
