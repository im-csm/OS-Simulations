...
int bestFit(int* blocks, int* block_count, int* mem, int d, double v, int n){
    if(head == -1)//no hole is found!
        return 0;
    int size = nrand_gen(d, v, n);  // Generate random size for block
    int iterator = head, pred, succ, new_size, block_address, bestHole = -1, bestSize = n;

    // Find the best fit for the new block
    while(1) {
        if (-mem[iterator] < bestSize)  // Hole size is smaller than the best size
            bestSize = -mem[iterator];        // reassign bestSize to the iterator
        if (size < bestSize)        // Block size is smaller than best sized hole
            bestHole = iterator;    // bestHole set to index of best sized hole
...
int main(int argc, char** argv) {
    ...
    while(x > 0){
        while(bestFit(blocks, &block_count, mem, d, v, n));
